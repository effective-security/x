...
- `${VAR}` uses `Expander.Variables` first, then `os.Getenv`. Unresolved
  `${...}` after expansion is an error. Bare `os.Getenv` misses become empty
  strings, matching `os.Expand`.
...
[`configloader/testdata/`](../configloader/testdata/). The README install path
(`…/x/pkg/configloader`) is stale; the module path is
...
- `Parse` splits on `|` or `,` and bitwise-ORs named values. Unknown names
  contribute `0`.
- Flag helpers walk bits `1, 2, 4, …` and skip unnamed bits.
...
| [`reloader/reloader.go`](../fileutil/reloader/reloader.go) | Poll mtime; `NewReloader`, `Reload`, `Close`, `LoadedAt`, `LoadedCount`. Callback runs on a new goroutine. `Close` errors if already closed. |
...
[`flake/README.md`](../flake/README.md). Trust the constants below, not the
stale bit counts in the `flake.go` package comment.
...
- `NewIDGenerator` and `NextID` **panic** on bad start time, machine-ID
  ...
  those as errors.
- Default machine ID: in test/debug binaries (`os.Args[0]` contains `test`
  or `debug`), FNV-32 of the program path; otherwise lower 16 bits of the
  first non-loopback interface address. Missing interfaces → ID `0`, not an
  error.
...
`Split` keeps acronyms (`ID`, `HTML`) and plural acronyms (`IDs`).

### `guid`

[`guid.go`](../guid/guid.go): `MustCreate()` → `XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX`
(uppercase hex). Panics if `crypto/rand` fails. Not RFC-4122 versioned; raw
random bytes. `randRead` is overridable in tests.
...
- `Print` / `Object` order: registered `CustomFn`, then `Printer` interface,
  then `[]string` / `map[string]string`, else JSON.
...
| [`coalesce.go`](../values/coalesce.go) | `StringsCoalesce`, `NumbersCoalesce`, `Coalesce`, `Select` |
...
- `MapAny.Add` skips empty keys and empty values; may allocate if receiver is nil.
- `MapAny.Slice` type-asserts to `[]any` and panics on mismatch.
- `Coalesce` on an empty vararg panics (`args[0]`).
...
- `XXH3HashArgs*`: strings are length-prefixed; string elements also get a
  trailing `0x00`. Unknown types go through `String(v)`.
...
Overridable time/rand hooks for tests: `flake.NowFunc`, `format.NowFunc`,
`guid` `randRead`, `reloader` `makeTicker`.
